...
public class FTP {
	...
	public synchronized boolean stor(File file) throws IOException {
		if (file.isDirectory()) { throw new IOException("FTP cannot upload a directory."); }
	...
	public synchronized boolean stor(InputStream inputStream, String filename) throws IOException {
		...
		if (!response.startsWith("150 ")) { throw new IOException("FTP was not allowed to send the file: " + response); }
	...
	public synchronized String[] ls() throws IOException {
		...
		if (!response.startsWith("150 ")) { throw new IOException("Could not read the directory"); }
		response = readLine();
		if (!response.startsWith("226 ")) { throw new IOException("Could not read the directory"); }
	...
	public synchronized boolean delete(String filename) throws IOException {
		...
		if (!response.startsWith("250 ")) { throw new IOException("FTP is not allowed to delete this file"); }
		return true;
	}
	...
	private synchronized Socket passive() throws IOException {
		...
		if (!response.startsWith("227 ")) { throw new IOException("FTP could not request passive mode: " + response); }
