...
public class Backups extends Thread {
	...
	public void compressDir(File tempDir, World world) {
		...
		if (this.plugin.config.compressionEnabled) {
			...
			this.plugin.getServer().getScheduler().scheduleAsyncDelayedTask(this.plugin, new ZipDir(this.plugin, this, tempDir.getPath(), dir + "/" + file + ".zip"));
		}
		else {
			this.plugin.log(Level.INFO, "\tCopying...");
			this.plugin.getServer().getScheduler().scheduleAsyncDelayedTask(this.plugin, new CopyDir(this.plugin, this, tempDir.getPath(), dir + "/" + file));
	...
	private void copyWorld(World world, File tempDir) throws IOException {
		...
		try {
			alexoft.Minebackup.DirUtils.copyDirectory(new File(world.getName()), new File(tempDir.getPath() + "/" + world.getName()));
		}
		catch (Exception ex) {
	...
	private void SendMessage(String m) {
		if (this.plugin.config.msg_enable) plugin.log(Level.INFO, "message count: " + plugin.getServer().broadcastMessage(m));
	}
