...
public class MineBackup extends JavaPlugin {
	...
	public void onEnable() {
		try {
			...
			if (this.config.pauseWhenNoPlayers) this.getServer().getPluginManager().registerEvent(Event.Type.PLAYER_JOIN, new MineBackupPlayerListener(this), Priority.Monitor, this);
	...
	public void resetSchedule() {
		...
		if (this.config.daystokeep != 0) {
			this.getServer().getScheduler().scheduleAsyncRepeatingTask(this, new BackupsCleaner(this), 0, this.config.interval * 2);
		}
		this.taskID = this.getServer().getScheduler().scheduleSyncRepeatingTask(this, new Backups(this), this.config.firstDelay, this.config.interval);
	}
	...
	public class LogFilter implements Filter {
		...
		public boolean isLoggable(LogRecord record) {
			if (isBackupStarted) {
				return !record.getMessage().equals(LS1) & !record.getMessage().equals(LS2) & !record.getMessage().equals(LS3) & !record.getMessage().equals(LS4) & !record.getMessage().equals(LS5) & !record.getMessage().equals(LS6) & !record.getMessage().equals(LS7) & !record.getMessage().equals(LS8);
			}
			else {
				return true;
			}
